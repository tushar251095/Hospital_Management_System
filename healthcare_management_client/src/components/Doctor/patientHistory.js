...
useEffect(()=>{
    axios.get("/history/"+localStorage.getItem("patientId"))
    .then((result)=>{
                    ...
                    <tbody>
                        {
                            record.map((rec,index)=>(
                                <tr key={index}>
                                    <td>{rec.doctordetails.firstName + " " + rec.doctordetails.lastName}</td>
                                    <td>{new Date(rec.appointmentTimestamp).toLocaleString()}</td>
                                    <td>{rec.status}</td>
                                    <td>
                                        <div className='text-center'>
                                            {
                                                rec.status=='attended' && 
                                                <button className='btn btn-info btn-sm' onClick={(e)=>showData(e,rec)}>View Details</button>
                                            }
                                            {
                                                rec.status!='attended' && 
          ...
          <div>
                <table className='table table-bordered table-striped'>
                    <tbody>
                        <tr>
                            <th>Doctor Name</th>
                            <td>{appdata.doctordetails.firstName+" "+appdata.doctordetails.lastName}</td>
                        </tr>
                        <tr>
                            <th>Date</th>
                            <td>{new Date(appdata.appointmentTimestamp).toLocaleString()}</td>
                        </tr>
                        <tr>
                            <th>Height</th>
